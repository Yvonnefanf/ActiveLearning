...
if __name__ == "__main__":
    ...
    file_path = os.path.join("..", "..", "..", "training_dynamic", "image", "AL_dataset")
    # log_path = os.path.join(file_path, "log")
    os.system("mkdir -p {}".format(file_path))
    # os.system("mkdir -p {}".format(log_path))
    # sys.stdout = open(os.path.join(log_path, now+".txt"), "w")
    ...
    if RESUME:
        ...
        # 加载索引列表
        index_file = '/home/yiming/cophi/training_dynamic/image/AL_dataset/Model/index.json'
        with open(index_file, 'r') as file:
            index_list = json.load(file)

        idxs_lb = np.array(index_list)

    print('number of labeled pool: {}'.format(len(idxs_lb)))
    print('number of unlabeled pool: {}'.format(n_pool - len(idxs_lb)))
    ...
    if SAVE:
        save_task_model(0, strategy, file_path)

    q_time = np.zeros(NUM_ROUND)
    t_time = np.zeros(NUM_ROUND)
    state_dict = torch.load(os.path.join("/home/yiming/cophi/training_dynamic/image/AL_dataset/Model", "Epoch_1", "subject_model.pth"))
    task_model.load_state_dict(state_dict)
    ...
    for rd in range(1, NUM_ROUND+1):
        ...
        # # query new samples
        # t0 = time.time()
        # new_indices, _ = strategy.query(embedding, NUM_QUERY)
        ...
        new_indices_file = '/home/yiming/cophi/training_dynamic/image/AL_dataset/Model/new_index.json'
        with open(new_indices_file, 'r') as file:
            new_indices_list = json.load(file)

        new_indices = np.array(new_indices_list)
        print('number of labeled pool: {}'.format(len(new_indices)))
        strategy.update_lb_idxs(new_indices)
        resnet_model = resnet18()
        strategy.train(total_epoch=1, task_model=task_model, complete_dataset=train_dataset)
        t2 = time.time()
